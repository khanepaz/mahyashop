...
function baleOrderUrl(orderId) {
  ...
  return orderId ? base + '?start=order_' + encodeURIComponent(orderId) : base;
}
...
function showSuccess(order) {
  ...
  document.getElementById('successMsg').textContent = lastOrderId
    ? ('کد سفارش: ' + lastOrderId + '\nبرای پیگیری و پرداخت، ربات بله را باز کنید.')
    : 'سفارش شما ثبت شد.';
...
async function submitOrder(e) {
  ...
  try {
    var res = await fetchWithTimeout(API_URL, {
      ...
      body: JSON.stringify({
        action: 'orders.create', customer: { name: name, phone: phone, address: address }, note: note,
  ...
  } catch (err) { toast(err.message || 'خطا در ثبت سفارش', 'err'); }
  finally { btn.disabled = false; btn.textContent = 'تأیید نهایی سفارش'; }
}
function applyFilters() {
  currentSort = document.getElementById('sortSelect').value;
  currentCategory = document.getElementById('filterCategory').value || 'all';
  var minV = document.getElementById('priceMin').value, maxV = document.getElementById('priceMax').value;
